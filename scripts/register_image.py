...
def register(verifier_address, image_key, rpc_url, private_key):
...
def main():
    ...
    print(f"\n  Measurements:")
    print(f"    MRTD:    {measurements.get('mrtd', b'').hex()[:32]}... (not in key)")
    print(f"    RTMR[0]: {measurements.get('rtmr0', b'').hex()[:32]}... (not in key)")
    ...
    register(args.verifier, image_key, args.rpc_url, args.private_key)
