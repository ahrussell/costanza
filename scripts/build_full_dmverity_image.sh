...
tar czf /tmp/tee-upload.tar.gz -C "$PROJECT_ROOT" tee/enclave/ agent/prompts/ 2>/dev/null
vm_scp "/tmp/tee-upload.tar.gz" "/tmp/"
vm_run "cd /tmp && tar xzf tee-upload.tar.gz && sudo cp -r tee/enclave /opt/humanfund/ && sudo cp prover/prompts/system.txt /opt/humanfund/system_prompt.txt 2>/dev/null || true"
