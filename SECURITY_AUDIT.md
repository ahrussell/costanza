...
3. **Donor message injection**: Five layers of defense: datamarking spotlighting with seed-derived markers (unpredictable at message submission), `sanitize_thinking()` stripping XML-like tags from reasoning propagation, 280-char message limit, 0.005 ETH minimum per message, and contract bounds (max 10% donation per epoch).
