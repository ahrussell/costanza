...
def build_epoch_context(state):
    ...
    lines.append(f"Liquid treasury: {format_eth(balance)} ETH")
    if total_invested > 0:
    ...
    lines.append(f"Total inflows: {format_eth(state.get('total_inflows', 0))} ETH")
    lines.append(f"Total donated to nonprofits: {format_eth(state.get('total_donated', 0))} ETH")
    ...
    lines.append(f"Inflows: {format_eth(state.get('epoch_inflow', 0))} ETH ({state.get('epoch_donation_count', 0)} donations)")

    # ── Section 2: Concrete Action Bounds ─────────────────────────────
    lines.append("")
    lines.append("=== YOUR ACTIONS THIS EPOCH ===")
    lines.append("")
    lines.append(f"  donate(nonprofit_id, amount_eth)        max: {format_eth(bounds['max_donate'])} ETH")
    ...
    if bounds['invest_capacity'] > 0:
        lines.append(f"  invest(protocol_id, amount_eth)          capacity: {format_eth(bounds['invest_capacity'])} ETH (max {format_eth(bounds['max_per_protocol'])} per protocol)")
    else:
        lines.append(f"  invest(protocol_id, amount_eth)          BLOCKED — at investment or reserve limit")
    if bounds['withdrawable']:
        withdraw_parts = [f"#{w['id']} {w['name']}: {format_eth(w['value'])}" for w in bounds['withdrawable']]
        ...
        for inv in state["investments"]:
            ...
            if inv["shares"] > 0:
                ...
                lines.append(
                    ...
                    f"{format_eth(inv['deposited'])} deposited -> {format_eth(inv['current_value'])} ({profit_str})"
                )
            else:
    ...
    lines.append("")
    lines.append(f"You are Constanza, epoch {epoch}. Liquid: {format_eth(balance)} ETH. Total assets: {format_eth(total_assets)} ETH.")
    if lifespan["epochs_remaining"] is not None:
        lines.append(f"Estimated lifespan: ~{lifespan['epochs_remaining']} epochs at current burn rate.")
    lines.append(f"Max donate: {format_eth(bounds['max_donate'])} ETH. Commission: {commission / 100:.1f}%.")
    lines.append(f"Total donated lifetime: {format_eth(state.get('total_donated', 0))} ETH. Epochs since last donation: {epochs_since_donation}.")
